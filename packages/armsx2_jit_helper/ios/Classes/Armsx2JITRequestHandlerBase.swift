...
private let armsx2RequestType = "com.neogamelab.neostation.armsx2-jit-request"

/// Dedicated out-of-process JIT helper for NeoStation's embedded ARMSX2 engine.
///
/// The helper attaches StikJIT to the NeoStation host PID with universal.js.
/// It never launches, inspects, or communicates with a standalone ARMSX2 app.
@available(iOS 17.4, *)
open class Armsx2JITRequestHandlerBase: NSObject, NSExtensionRequestHandling {
  private let jitQueue = DispatchQueue(
    label: "com.neogamelab.neostation.armsx2.jit-helper",
    qos: .userInitiated
  )
  ...
  open func beginRequest(with context: NSExtensionContext) {
    guard
      ...
      let provider = item.attachments?.first(where: {
        $0.hasItemConformingToTypeIdentifier(armsx2RequestType)
      })
    else {
      context.cancelRequest(
        withError: Armsx2HelperError.invalidRequest(
          "Missing ARMSX2 JIT request payload."
        )
      )
      return
    }
    ...
    provider.loadItem(
      forTypeIdentifier: armsx2RequestType,
      options: nil
      ...
      guard let data else {
        context.cancelRequest(
          withError: Armsx2HelperError.invalidRequest(
            "Unreadable ARMSX2 JIT request payload."
          )
        )
        return
      }
  ...
  private func process(data: Data, context: NSExtensionContext) {
    var reporter: Armsx2HelperReporter?
    ...
    do {
      guard
        ...
        throw Armsx2HelperError.invalidRequest(
          "Invalid ARMSX2 JIT request fields."
        )
      }
      ...
      guard let probeNonce = object["probeNonce"] as? NSNumber,
        ...
        throw Armsx2HelperError.invalidRequest("Missing debugger probe nonce.")
      }
      // ARMSX2 selects its mirrored Universal arena by OS version, not TXM
      ...
      reporter = try Armsx2HelperReporter(
      ...
      try reporter?.send(
        ...
        message: "ARMSX2 JIT helper connected to NeoStation."
      )
      try reporter?.send(
        event: "log",
      ...
      let temporaryDirectory = FileManager.default.temporaryDirectory
        .appendingPathComponent("NeoStationARMSX2JIT", isDirectory: true)
      ...
      try reporter?.send(
        event: "log",
        message: "Starting StikJIT universal.js for the in-process ARMSX2 engine."
      )

      var script = StikJIT.Script.universal
      if requiresCoreHandshake {
        let resource = Bundle(for: Armsx2JITRequestHandlerBase.self)
          .url(forResource: "armsx2-universal", withExtension: "js") ??
          Bundle.main.url(forResource: "armsx2-universal", withExtension: "js")
        guard let resource else {
          throw Armsx2HelperError.invalidRequest("The ARMSX2 debugger handshake script is missing.")
        }
      ...
      try reporter?.send(
        event: "complete",
        message: "StikJIT completed the ARMSX2 universal transaction and detached.",
        success: true
      )
...
private final class Armsx2HelperReporter {
  private let connection: NWConnection
  private let queue = DispatchQueue(
    label: "com.neogamelab.neostation.armsx2.jit-helper-reporter"
  )
  ...
  init(port: UInt16, token: String) throws {
    guard let endpointPort = NWEndpoint.Port(rawValue: port) else {
      throw Armsx2HelperError.invalidRequest("Invalid NeoStation helper port.")
    }
    self.token = token
  ...
  func connect() throws {
    ...
    guard semaphore.wait(timeout: .now() + 20) == .success else {
      throw Armsx2HelperError.connection(
...
private enum Armsx2HelperError: LocalizedError {
