...
class ToolsSettingsContentState extends State<ToolsSettingsContent> {
  ...
  bool _useArmsx2DirectLaunch = false;
  ...
  void selectItem(int index) {
    ...
    if (index == 2 &&
      ...
      _setUseArmsx2DirectLaunch(!_useArmsx2DirectLaunch);
    }
  }
  ...
  Future<void> _refreshArmsx2LaunchModeState() async {
    try {
      ...
      if (!mounted) return;
      setState(() {
        _useArmsx2DirectLaunch = enabled;
  ...
  Future<void> _setUseArmsx2DirectLaunch(bool value) async {
    if (!_armsx2LaunchModeStateLoaded || _isUpdatingArmsx2LaunchMode) return;

    final previousValue = _useArmsx2DirectLaunch;
    setState(() {
      _useArmsx2DirectLaunch = value;
      _isUpdatingArmsx2LaunchMode = true;
    });

    try {
      await JitBackendPreferenceService.setUseArmsx2DirectLaunch(value);
      if (!mounted) return;
      AppNotification.showNotification(
        context,
      ...
      if (!mounted) return;
      setState(() => _useArmsx2DirectLaunch = previousValue);
      AppNotification.showNotification(
        context,
