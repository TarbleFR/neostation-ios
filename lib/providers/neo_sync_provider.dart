...
class NeoSyncProvider extends ChangeNotifier {
  ...
  /// Runs an iOS ARMSX2-only upload pass.
  ///
  /// This deliberately does not call [autoSyncUploads]: that global routine
  /// also scans RetroArch, MeloNX and RPCS3. ARMSX2 cold-return recovery must
  /// never upload a Switch/RetroArch/PS3 file just because those roots also
  /// contain changes. Only memcards/savestates/sstates under the linked ARMSX2
  /// root are considered here, and every file is routed through the existing
  /// ARMSX2 uploader (system=ps2, emulator=armsx2, scope=shared).
  Future<bool> autoSyncArmsx2UploadsOnly() async {
    if (!Platform.isIOS || !isNeoSyncAuthenticated || _isSyncing) {
      return false;
    }
    ...
    _error = null;
    _syncProgress = 0.0;
    _syncStatus = 'Checking ARMSX2 saves...';
    _totalFiles = 0;
    _processedFiles = 0;
    _uploadedFiles = 0;
    _skippedFiles = 0;
    _downloadedFiles = 0;
    _processedItems = ['ARMSX2-only NeoSync pass'];
    notify();

    try {
      const categories = <String>['memcards', 'savestates', 'sstates'];
      final selectedName = path.basename(armsx2Root).toLowerCase();
      final roots = <({String folder, bool isState})>[];

      if (categories.contains(selectedName)) {
        roots.add((folder: armsx2Root, isState: selectedName != 'memcards'));
      } else {
        for (final category in categories) {
          final folder = path.join(armsx2Root, category);
          if (Directory(folder).existsSync()) {
            roots.add((folder: folder, isState: category != 'memcards'));
          }
        }
      }

      final files = <({File file, bool isState})>[];
      for (final root in roots) {
        await for (final entity in Directory(root.folder).list(
          recursive: true,
          followLinks: false,
        )) {
          if (entity is File) {
            files.add((file: entity, isState: root.isState));
          }
        }
      }
      ...
      if (files.isEmpty) {
        _syncProgress = 1.0;
        _syncStatus = 'No ARMSX2 save files found';
        _processedItems.add('No ARMSX2 memory card or save state found');
        notify();
        return true;
      }

      _syncStatus = 'Uploading ARMSX2 saves...';
      notify();

      var allSuccessful = true;
      for (final entry in files) {
        final uploaded = await NeoSyncUpload(this)._uploadArmsx2File(
          entry.file,
          armsx2Root,
        );
        if (!uploaded) allSuccessful = false;
        _processedFiles++;
      ...
      _processedItems.add(
        'ARMSX2-only sync completed: $_uploadedFiles uploaded, $_skippedFiles already synced',
      );
      notify();
      return allSuccessful && _error == null;
    } catch (e) {
      _error = 'ARMSX2 sync error: $e';
      _syncStatus = _error!;
      _processedItems.add(_error!);
      notify();
      return false;
    } finally {
      _setSyncing(false);
