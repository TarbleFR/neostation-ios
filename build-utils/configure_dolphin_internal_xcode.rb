...
# Link the device framework from a SOURCE_ROOT-relative path. Putting this
# reference inside a Frameworks group would make Xcode resolve it below
# ios/Frameworks rather than from ios/Runner.xcodeproj's directory.
stik_path = '../packages/stikjit_bridge/ios/Frameworks/StikJIT.xcframework/ios-arm64/StikJIT.framework'
stik_ref = project.main_group.files.find { |file| file.path == stik_path }
stik_ref ||= project.main_group.new_file(stik_path)
