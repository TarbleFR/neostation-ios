#!/usr/bin/env python3
"""Build 263 full-theme integration patch.

Keeps Build 260 as the source baseline while applying the full-theme additions
before Flutter/Xcode compilation. The local JIT VPN intentionally remains on
the proven Build 259 implementation. The patch is idempotent because the
existing CI applies host patchers twice to detect drift.
"""
...
def patch_vpn_build_configuration() -> None:
    ...
    if "com.apple.developer.networking.vpn.api" in text or "allow-vpn" in text:
        raise SystemExit("Build 263 must not mix Personal VPN and Packet Tunnel")
    text = text.replace(
        "ENV.fetch('BUILD_NUMBER', '260')",
        "ENV.fetch('BUILD_NUMBER', '263')",
    )
    write(relative, text)
...
def patch_full_theme_service() -> None:
    ...
    if "arcadePlanetRevision" not in text:
        ...
        if anchor not in text:
            raise SystemExit("Build 263 full-theme constant anchor drifted")
    ...
    if "Future<FullThemeDefinition> downloadArcadePlanet" not in text:
        ...
        if anchor not in text:
            raise SystemExit("Build 263 full-theme method anchor drifted")
...
def patch_full_theme_locale() -> None:
    ...
    if "_downloadArcadePlanet" not in text:
        ...
        if anchor not in text:
            raise SystemExit("Build 263 full-theme locale anchor drifted")
    ...
    if "static String downloadArcadePlanet" not in text:
        if static_anchor not in text:
            raise SystemExit("Build 263 full-theme locale method anchor drifted")
...
def patch_full_theme_settings() -> None:
    ...
    if "_fullThemeDownloadInProgress" not in text:
        ...
        if old not in text:
            raise SystemExit("Build 263 settings state anchor drifted")
    ...
    if "await _showFullThemeInstallActions();" not in text:
        if old_null not in text:
            raise SystemExit("Build 263 full-theme empty-state anchor drifted")
    ...
    if "Future<void> _showFullThemeInstallActions()" not in text:
        ...
        if anchor not in text:
            raise SystemExit("Build 263 full-theme actions anchor drifted")
...
'''
    ...
    if "FullThemeLocale.downloadArcadePlanet(dialogContext)" in text and text.count(
        ...
        new = """              ListTile(\n                leading: const Icon(Symbols.download_rounded),\n                title: Text(\n                  FullThemeLocale.downloadArcadePlanet(dialogContext),\n                ),\n                onTap: _fullThemeDownloadInProgress\n                    ? null\n                    : () {\n                        Navigator.of(dialogContext).pop();\n                        _downloadArcadePlanet();\n                      },\n              ),\n              ListTile(\n                leading: const Icon(Symbols.folder_open_rounded),\n                title: Text(FullThemeLocale.replace(dialogContext)),\n"""
        if old not in text:
            raise SystemExit("Build 263 active full-theme action anchor drifted")
        text = text.replace(old, new, 1)

    if "Future<void> _downloadArcadePlanet()" not in text:
        anchor = "  Future<void> _pickFullTheme() async {\n"
        if anchor not in text:
            raise SystemExit("Build 263 full-theme picker anchor drifted")
...
def main() -> None:
    ...
    print("NeoStation Build 263 VPN/full-theme integration patch applied")
