#!/usr/bin/env python3
"""Inspect Build 274 RPCS3 launch-stability IPA; packaging success is not iPhone validation."""
...
def verify(path):
 with zipfile.ZipFile(path) as archive:
  ...
  assert str(info['CFBundleVersion'])=='274','Wrong application build'
  ...
  return {
   ...
   'vpnPolicy':'manual ON/OFF only; one bounded stable route gate before JIT; no VPN mutation during RPCS3 boot',
   'onDemand':False,
