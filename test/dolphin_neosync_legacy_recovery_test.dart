...
void main() {
  ...
  Future<DolphinSaveSnapshot> fixture(DolphinSaveIdentity game) async {
    ...
    if (game.system == 'gc') {
      await put(target.relativeNativePath, Uint8List(524288)..[0] = 17);
    } else {
  ...
  for (final game in [gc, wii]) {
    test('${game.system} old typed object recovers only from exact native snapshot bytes', () async {
      ...
      for (final wrong in [
        ...
        historical(snapshot, override: {
          'scope': snapshot.target.shared ? 'game' : 'shared',
        }),
  ...
  test('repeated raw object is unique and another region cannot steal it', () async {
    ...
    final other = DolphinSaveTarget.forGame(const DolphinSaveIdentity(
        system: 'gc', gameId: 'GZLE01', region: 'USA')).first;
    index.add(path: other.cloudPath, leaf: other.objectName,
        size: snapshot.size, checksum: snapshot.checksum);
    expect(index.resolve(old).verifiedSourcePath, snapshot.target.cloudPath);
  });
  ...
  test('known native game identity and console cannot be reassigned by matching hash', () async {
    ...
    final old = historical(snapshot, override: {
      'file_name': snapshot.target.cloudPath.replaceFirst(
        snapshot.target.identity,
        'Different title',
      ),
      'file_path': snapshot.target.cloudPath.replaceFirst(
        snapshot.target.identity,
        'Different title',
      ),
    });
