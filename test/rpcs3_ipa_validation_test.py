...
class RPCS3IPAValidationTests(unittest.TestCase):
    def test_required_runtime_contract_includes_input_export(self):
        self.assertEqual(validator.CORE_NAME, 'libRPCS3Core.dylib')
        self.assertIn('_rpcs3_ios_set_pad_state', validator.REQUIRED_CORE_SYMBOLS)
        self.assertIn('_rpcs3_ios_boot_game', validator.REQUIRED_CORE_SYMBOLS)
        self.assertIn('_rpcs3_ios_run_llvm_self_test', validator.REQUIRED_CORE_SYMBOLS)
    ...
    def test_rejects_packaged_app_without_rpcS3_core_before_native_inspection(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with self.assertRaisesRegex(validator.ValidationError, 'RPCS3 core missing'):
                validator.validate_ipa(ipa, '242', 'deadbeef')
