...
## Ajout demandé : enregistrement vidéo à 50 images/s

La vidéo fournie `ScreenRecording_09-06-2026 01-12-56_1.mov` contient 1 691 images sur 32,248 s, soit 52,44 images/s en moyenne. Les timestamps sont variables : 208 intervalles dépassent 25 ms, le maximum atteint 200 ms. Entre 18 et 31 s, l’intervalle moyen est de 22,69 ms et le 95e percentile de 33,33 ms. Le graphe visible en course affiche environ 42 FPS / 70 % de vitesse avec une résolution interne de 2560 × 2112 (4×). Ces observations prouvent un ralentissement de l’émulation pendant la capture ; elles ne permettent pas d’isoler à elles seules CPU, GPU et pression thermique.

Un enregistrement intégré est proposé dans le menu Dolphin : ReplayKit fournit les images et le son du jeu, AVAssetWriter écrit une vidéo H.264 à timestamps espacés de 20 ms, bornée au format 1920 × 1080 en conservant le rapport d’image. Le microphone reste désactivé. Le traitement est asynchrone avec files et buffers bornés, et les vidéos sont enregistrées dans `Documents/Recordings/Dolphin`, hors des racines NeoSync. Le partage reste une action explicite.

Pendant cet enregistrement, seule la résolution interne effective est plafonnée temporairement à 2× (1× conservé si déjà choisi). Les préférences persistantes restent intactes ; une nouvelle préférence explicitement choisie est respectée après l’arrêt. Le plafond est rétabli à la fin, en cas d’échec et lors de la fermeture de la session. Aucun changement de vitesse d’émulation ni seconde boucle MTKView n’est ajouté.

La sortie à 50 FPS impose un rééchantillonnage : un jeu à 60 FPS perd périodiquement une image ; une source sous 50 FPS nécessite des répétitions. Ce mécanisme ne crée aucune image intermédiaire et ne constitue pas une garantie de zéro saccade. La capture du Centre de contrôle iOS ne reçoit pas ce réglage ; il faut utiliser l’enregistrement intégré.

Sources Apple consultées le 5 septembre 2026 : [ReplayKit startCapture](https://developer.apple.com/documentation/replaykit/rpscreenrecorder/startcapture(handler:completionhandler:)), [AVVideoExpectedSourceFrameRateKey](https://developer.apple.com/documentation/avfoundation/avvideoexpectedsourceframeratekey) (indice encodeur, pas commande de cadence), [append withPresentationTime](https://developer.apple.com/documentation/avfoundation/avassetwriterinputpixelbufferadaptor/append(_:withpresentationtime:)), [isReadyForMoreMediaData](https://developer.apple.com/documentation/avfoundation/avassetwriterinput/isreadyformoremediadata), [ProMotion](https://developer.apple.com/documentation/quartzcore/optimizing-iphone-and-ipad-apps-to-support-promotion-displays). Les capacités disponibles dans Xcode 16.4/iOS 17.4 sont utilisées ; aucun accès à une API de SDK ultérieur n’est supposé.
