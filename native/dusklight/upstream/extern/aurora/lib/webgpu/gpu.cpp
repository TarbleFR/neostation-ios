...
void release_frame_targets() {
  ...
  g_CopyBindGroup = {};
  const auto destroy = [](TextureWithSampler& target) {
    if (target.texture) target.texture.Destroy();
    target = {};
  };
  destroy(g_frameBuffer);
  destroy(g_frameBufferResolved);
  destroy(g_depthBuffer);
  destroy(g_normalBuffer);
  destroy(g_resampledFrameBuffer);
  // Keep pipelines, samplers, configuration and the game's persistent textures.
  // aurora::begin_frame refreshes a missing surface before recording resumes.
}
