...
void map_staging_buffer(size_t slot, bool releaseSlotOnCompletion = false) {
  ...
  g_mappingFutures[slot] = g_stagingBuffers[slot].MapAsync(
...
static void create_frame_buffers() {
...

  {
    ...
    if (!g_resources.staticBindGroupLayout) {
...

  {
    ...
    if (!g_resources.uniformBindGroupLayout) {
...
// Host-only frame boundary: retain the game, device and persistent textures,
// but surrender scratch GPU storage while another core owns the foreground.
// A worker barrier alone does not wait for submitted GPU work or MapAsync.
bool release_frame_resources() {
  if (g_frameBuffersReleased || g_frameIndex == UINT32_MAX) return true;
  render_worker::synchronize();
  const auto deadline = PresentClock::now() + std::chrono::seconds{1};
  auto completed = std::make_shared<std::atomic_bool>(false);
  const auto submitted = g_queue.OnSubmittedWorkDone(
      wgpu::CallbackMode::AllowSpontaneous,
      [completed](wgpu::QueueWorkDoneStatus status, wgpu::StringView) {
        completed->store(status == wgpu::QueueWorkDoneStatus::Success, std::memory_order_release);
      });
  const auto wait = [&](wgpu::Future future) {
    if (!future.id) return true;
    const auto remaining = deadline - PresentClock::now();
    if (remaining <= PresentClock::duration::zero()) return false;
    return g_instance.WaitAny(future, static_cast<uint64_t>(duration_ns(remaining))) == wgpu::WaitStatus::Success;
  };
  if (!wait(submitted) || !completed->load(std::memory_order_acquire)) return false;
  for (const auto future : g_mappingFutures) {
    if (!wait(future)) return false;
  }
  // Every old mapping callback has now returned. No late callback can release
  // a slot belonging to a subsequent session, nor access a destroyed buffer.
  clear_caches();
  shutdown_recording();
  g_resources.staticBindGroup = {};
  g_resources.uniformBindGroup = {};
  const auto destroy = [](wgpu::Buffer& buffer) {
    if (buffer) buffer.Destroy();
    buffer = {};
  };
  destroy(g_resources.vertexBuffer);
  destroy(g_resources.uniformBuffer);
  destroy(g_resources.indexBuffer);
  destroy(g_resources.storageBuffer);
  for (auto& buffer : g_stagingBuffers) destroy(buffer);
  for (auto& packet : g_framePackets) packet = {};
  for (auto& state : g_mappingStates) state.store(BufferMapState::Unmapped, std::memory_order_release);
  g_mappingFutures.fill({});
  g_frameSlots.reset();
  g_stagingSlots.reset();
  webgpu::release_frame_targets();
  g_frameBuffersReleased = true;
  return true;
}

static void resume_frame_resources() {
  if (!g_frameBuffersReleased) return;
  create_frame_buffers();
  g_frameBuffersReleased = false;
  g_lastPresentNs.store(0, std::memory_order_release);
  g_presentPeriodNs.store(0, std::memory_order_release);
