...
void MenuBar::build_tabs() {
    ...
    mTabBar->add_tab(NeoDusklight_UIText("returnToLibrary"), [this] {
        mTabBar->set_active_tab(-1);
        const auto dismiss = [](Modal& modal) { modal.pop(); };
        push(std::make_unique<Modal>(Modal::Props{
            .title = NeoDusklight_UIText("returnToLibrary"),
            .bodyText = NeoDusklight_UIText("resumeHint"),
            .actions =
                {
                    ModalAction{
                        .label = NeoDusklight_UIText("cancelReturn"),
                        .onPressed =
                            [dismiss](Modal& modal) {
                                mDoAud_seStartMenu(kSoundWindowClose);
                                dismiss(modal);
                    ...
                    ModalAction{
                        .label = NeoDusklight_UIText("returnToLibrary"),
                        .onPressed =
                            [dismiss](Modal& modal) {
                                mDoAud_seStartMenu(kSoundClick);
                                dismiss(modal);
