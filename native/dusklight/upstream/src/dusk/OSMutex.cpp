#include <chrono>
// NeoStation: retain synchronization objects through orderly shutdown.
...
void OSWaitCond(OSCond* cond, OSMutex* mutex) {
    ...
    if (savedCount >= 1) {
        ...
        // The caller rechecks its predicate after wakeup. A bounded wait also
        // observes shutdown if it raced with notification.
        condData.cv.wait_for(lock, std::chrono::milliseconds(50));
        lock.release();
        for (s32 i = 1; i < savedCount; i++) {
