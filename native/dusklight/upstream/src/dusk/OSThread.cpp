...
// NeoStation: wake and join native workers before renderer/window teardown.
    ...
    if (prevSuspend == 0) {
        ...
        if (data && data->started) {
            ...
            if (thread == OSGetCurrentThread()) {
                ...
                data->cv.wait(threadLock, [data] { return !data->suspended || dusk::IsShuttingDown.load(); });
            } else {
...
void OSSleepThread(OSThreadQueue* queue) {
    ...
    cv.wait(lock, [currentThread]() {
        return currentThread->state != OS_THREAD_STATE_WAITING || dusk::IsShuttingDown.load();
    });
}
...
extern "C" void NeoDusklight_WakeGameThreads() {
    {
        ...
        for (auto& [thread, data] : GetThreadDataMap()) {
            std::lock_guard threadLock(data->mtx);
            data->suspended = false;
            data->cv.notify_all();
        }
    }
    {
        ...
        for (auto& [queue, cv] : GetQueueCvMap()) cv->notify_all();
    }
}

extern "C" void NeoDusklight_JoinGameThreads() {
    std::vector<PCThreadData*> workers;
    {
        std::lock_guard lock(GetThreadDataMutex());
        for (auto& [thread, data] : GetThreadDataMap()) {
            if (data->nativeThread.joinable() &&
                data->nativeThread.get_id() != std::this_thread::get_id()) {
                workers.push_back(data.get());
            }
        }
    }
