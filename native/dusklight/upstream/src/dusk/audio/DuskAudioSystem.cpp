...
static bool HostAudioSuspended = false;
static bool HostHadAudio = false;
...
void dusk::audio::SetPaused(const bool paused) {
    if (!PlaybackStream || HostAudioSuspended) return;
    if (paused) {
        SDL_PauseAudioStreamDevice(PlaybackStream);
    } else {
        SDL_ResumeAudioStreamDevice(PlaybackStream);
...
extern "C" void NeoDusklight_SetAudioSuspended(int suspended) {
    if (HostAudioSuspended == bool(suspended)) return;
    HostAudioSuspended = bool(suspended);
    if (suspended) {
        HostHadAudio = PlaybackStream != nullptr;
        if (HostHadAudio) {
            SDL_PauseAudioStreamDevice(PlaybackStream);
            // Release the device while NeoStation or another core owns audio.
            // Preserve DSP/JAS state; Initialize() would allocate it twice.
            dusk::audio::Shutdown();
        }
    } else if (HostHadAudio) {
        InitSDL3Output();
        if (PlaybackStream) SDL_ResumeAudioStreamDevice(PlaybackStream);
    }
}
