...
static void ClearMsgQueueMap() {
    ...
    for (auto & [_, value] : map) {
        // Synchronize with the wait predicate before notification. The table
        // stays alive until all native workers have returned; no CV is freed
        // while a waiter may still reference it.
        std::lock_guard queueLock(value->mtx);
