...
// NeoStation owns the process-wide AVAudioSession. This private SDL backend
// owns only its AudioQueues and their interruption observers.
...
static bool UpdateAudioSession(SDL_AudioDevice *device, bool open, bool allow_playandrecord)
{
    @autoreleasepool {
        ...
        // NEOSTATION_DUSKLIGHT_HOST_AUDIO_OWNER_V1
        // Closing the last SDL queue must not deactivate NeoStation's audio
        // session: doing so stops the host's SoLoud device, not just our voices.
        // Likewise, do not replace the host's category/options on open. Host
        // policy owns activation at startup, foreground and interruption end.
        (void)allow_playandrecord;
